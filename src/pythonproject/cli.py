"""Console script for pythonproject."""
...
def serve(
    ...
    console.print(
        Panel(
            ...
            title="pythonproject Web服务",
            border_style="green",
        )
    )

    uvicorn.run("pythonproject.app:app", host=host, port=port, reload=reload)
...
def routes():
    """显示所有API路由。"""
    try:
        from pythonproject.app import app as fastapi_app
    except ImportError:
        console.print(
...
def info():
    """显示项目信息。"""
    version = getattr(pythonproject, "__version__", "未知")

    console.print(
        Panel(
            f"""
项目名称: [bold]pythonproject[/bold]
...
def version():
    """显示版本信息。"""
    version = getattr(pythonproject, "__version__", "未知")
    console.print(f"pythonproject v{version}")
